...
| `CONTENT_SOURCE` | No | Content source behind the `ContentService` port. Unset = the in-repo module adapter. `supabase` = published rows from `unfolding_entries`/`wiggling_voices` via the service-role client (the U7 admin-plane source). `atproto` (+ `CONTENT_ATPROTO_REPO`) = the gated spike read path. |
...
| `STRIPE_PRICE_ID_MONTHLY_SOLIDARITY` / `_STANDARD` / `_SUPPORTER` / `STRIPE_PRICE_ID_ANNUAL` / `STRIPE_PRICE_ID_LIFETIME` | Per offered cadence/tier | Fixed-price Stripe Price IDs, one per tier (separate Products, so Stripe reports subscriber counts per tier). Monthly is a solidarity scale — three tiers; annual/lifetime one each. A cadence/tier is offered only when its Price ID is set. |
...
- `unfolding_entries` / `wiggling_voices` — Newsletter essays and Wiggling voices as rows. Service-role-only (RLS on, no policies). **Edited at `/admin/unfolding`, not in the repo** once `CONTENT_SOURCE=supabase` is live; until the U8 cutover the module adapter (`src/lib/content/*.ts`) remains the default and the Supabase source is gated.
...
- **PWA install signals are device-verified, not CI-verified.** `src/app.html.test.ts` pins the head tags (both capability metas, the manifest link, and its agreement with `manifestFilename` in `vite.config.ts`) — that guard exists because a Chromium deprecation warning once prompted renaming `apple-mobile-web-app-capable` to the unprefixed name, which WebKit does not read, and iOS lost standalone display for six months without any build, `svelte-check`, or Lighthouse failure. Lighthouse runs in Chromium and cannot see WebKit-only breakage. After changing `app.html`, the `SvelteKitPWA` block, or the `@vite-pwa/sveltekit` version, spot-check on a real iOS device.
- `sw.js` is generated by the PWA plugin but never registered — there is no offline behaviour and no update path today. Registering it would switch on precaching of the whole client bundle, so treat it as its own change.
- `/manifest.webmanifest` exists only after a build (`devOptions.enabled` is false), so it 404s under `npm run dev`. Use `npm run build` plus `npx wrangler pages dev .svelte-kit/cloudflare` with a `Host:` header to verify PWA behaviour locally.
