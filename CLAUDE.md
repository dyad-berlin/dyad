...
| `STRIPE_SECRET_KEY` | Yes (Stripe-bearing surfaces) | Stripe API secret key. Test-mode (`sk_test_*`) for development; live-mode (`sk_live_*`) for production. Lives in Cloudflare Pages Production env vars, never in source. Consumed via `src/lib/server/stripe.ts`. |
| `STRIPE_WEBHOOK_SECRET` | Yes (Stripe-bearing surfaces) | Stripe webhook signing secret used by the `/api/stripe/webhook` endpoint to verify `Stripe-Signature` against the request body. Distinct from the API key: comes from the Stripe Dashboard's Webhooks page when the endpoint is registered. |
| `STRIPE_PRICE_ID_FOUNDING_CIRCLE` | Yes (Founding Circle pre-sale) | Product or Price identifier for the Founding Circle Stripe Checkout session. Set in the Stripe Dashboard once and pinned here. |
...
**Payment opacity contract.** Any Stripe-bearing code path lives behind `src/lib/server/stripe.ts`. dyad's database holds opaque tokens only (`stripe_customer_id`, `stripe_payment_intent_id`, `stripe_subscription_id`) plus the boolean / numeric state dyad needs (`amount_cents`, `kind`, timestamps). No card details, no wallet identities, no payer emails or names, no billing addresses are stored on dyad's side. Stripe holds the contact-bearing data per its data-controller role. Webhook handlers and any payment-bearing code path log only `event.id`, `event.type`, and `metadata.kind` — never `event.data.object` or sub-fields. dyad accepts paid lifetime memberships, never donations: see `~/.claude/projects/-home-digit-dyad-berlin/memory/project_donations_vs_membership.md` (memory) and the Founding Circle plan for the rationale.
